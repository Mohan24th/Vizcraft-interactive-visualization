...
def profile_dataset(dataset_id: str):
    ...
    try:
        ...
        profile["columns_count"] = int(len(df.columns))
        ...
        profile["total_missing_values"] = int(
            df.isnull().sum().sum()
        )
        ...
        # ---------------- DATA QUALITY SCORE ----------------
        missing_penalty = min(
            profile["total_missing_values"] * 0.1,
            30
        )
        ...
        raise Exception(
            f"Dataset profiling failed: {str(e)}"
        )